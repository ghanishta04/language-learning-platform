...
<head>
  ...
  <style>
    ...
    .card.marathi   { background: #fff5cc; }
    .card a {
      color: #333;
      text-decoration: none;
      font-weight: bold;
      ...
      <?php $i=1; foreach($result as $row): ?>
        <?php 
          $lessonLink = match (strtolower($course)) { 
            ...
            'marathi'   => 'marathi-lesson.php', // ✅ Added Marathi support
            'odia' => 'odia-lesson.php',
            'kannada' => 'kannada-lesson.php',
            'konkani' => 'konkani-lesson.php',
            'rajasthani' => 'rajasthani-lesson.php',
            'bengali' => 'bengali-lesson.php',
            'manipuri' => 'manipuri-lesson.php',
            default     => 'lesson-detail.php'
          };
        ?>
        <div class="card <?= htmlspecialchars($course) ?>">
          <h3>
            <a href="<?= $lessonLink ?>?lesson_id=<?= $row['id'] ?>&course=<?= urlencode(ucfirst($course)) ?>">
              <?= $i++ ?>. <?= htmlspecialchars($row['title']) ?>
            </a>
          </h3>
          <a href="<?= $lessonLink ?>?lesson_id=<?= $row['id'] ?>&course=<?= urlencode(ucfirst($course)) ?>" class="start-btn">Start Lesson</a>
        </div>
      <?php endforeach; ?>
    </div>
  <?php else: ?>
